...
// **** Addition & Modification for making Windows 64bit application by Silhwan Hyun (2016-05-15)
...
type
  ...
  {$if CompilerVersion > 21}  // Delphi XE 이후 버전이면
  TDVDTimeCode = record
    Hours1    : byte;
    Hours10   : byte;
    Minutes1  : byte;
...
// *****************************************************************************
  ...
  {@exclude}
  TFilCatNode = record
    FriendlyName : string; //SZ Shortstring;
    CLSID        : TGUID;
  end;
...
const
  ...
  NANOSECONDS_ = LONGLONG(1000000000);    // 10 ^ 9
  UNITS = LONGLONG(NANOSECONDS_ div 100); // 10 ^ 7
...
{$ENDIF}
 ...
 //----------------------------------------------------------------------------
  ...
  var
    width, height: DWORD;
  begin
    width := DWORD(Header.biWidth) * DWORD(Header.biBitCount);
    height := Abs(Header.biHeight);
    result := (((width + 31) and not 31) shr 3) * height;
  end;
 ...
 //----------------------------------------------------------------------------
  ...
  begin
    case Fourcc of
      ...
      PDWORD(@CC)^ := Fourcc; // Praise the Lord!
      result := string(CC);
    end;
  end;
...
// *****************************************************************************
  ...
  begin
    if catList.Count > 0 then
      for i := 0 to (catList.Count - 1) do if assigned(catList.Items[i]) then Dispose(PFilCatNode(catList.Items[i]));
    catList.Clear;
  ...
  begin
    ...
    if FCategories.Count > 0 then
      for i := 0 to (FCategories.Count - 1) do
        if assigned(FCategories.Items[i]) then Dispose(PFilCatNode(FCategories.items[i]));
    FCategories.Clear;
    FreeAndNil(FCategories);
    if FFilters.Count > 0 then
      for i := 0 to (FFilters.Count - 1) do
        if assigned(FFilters.Items[i]) then Dispose(PFilCatNode(FFilters.Items[i]));
    FFilters.Clear;
    FreeAndNil(FFilters);
  end;
...
begin
  ...
  begin
    ...
    if Assigned(ThisPinInfo.pFilter) then
    begin
      PinOut := nil;
...
begin
  ...
  begin
    tmp := Dividend.HighPart shl 32 + Dividend.LowPart;
    Remainder^ := ULONG(tmp mod Divisor);
    // Remainder^ := ULONG(LLtoU64(Dividend) mod Divisor);
  end;
  result := ULONG(tmp div Divisor);
...
begin
  ...
  else if IsEqualGuid(GUID,IID_IMediaSeeking) then Result := 'IID_IMediaSeeking'
  else if IsEqualGuid(GUID,IID_IMediaPosition) then Result := 'IID_IMediaPosition'
  else if IsEqualGuid(GUID,IID_IReferenceClock) then Result := 'IID_IReferenceClock'
  else if IsEqualGuid(GUID,IID_IAMFilterMiscFlags) then Result := 'IID_IAMFilterMiscFlags'
  else if IsEqualGuid(GUID,IID_IAMOpenProgress) then Result := 'IID_IAMOpenProgress'
  else if IsEqualGuid(GUID,IID_IAMDeviceRemoval) then Result := 'IID_IAMDeviceRemoval'
  else if IsEqualGuid(GUID,IID_IBasicVideo) then Result := 'IID_IBasicVideo'
  else if IsEqualGuid(GUID,IID_IVideoWindow) then Result := 'IID_IVideoWindow'
  else if IsEqualGuid(GUID,IID_IBasicAudio) then Result := 'IID_IBasicAudio'
  else if IsEqualGuid(GUID,IID_IStreamBuilder) then Result := 'IID_IStreamBuilder'
  else if IsEqualGuid(GUID,IID_IVMRSurface) then Result := 'IID_IVMRSurface'
  else if IsEqualGuid(GUID,IID_IBaseFilter) then Result := 'IID_IBaseFilter'
  else if IsEqualGuid(GUID,IID_IKsPropertySet) then Result := 'IID_IKsPropertySet'
  else if IsEqualGuid(GUID,IID_IMemInputPin) then Result := 'IID_IMemInputPin'
  else if IsEqualGuid(GUID,IID_IMediaSample2) then Result := 'IID_IMediaSample2'
  else if IsEqualGuid(GUID,IID_IMediaSample) then Result := 'IID_IMediaSample'
